...
typedef struct _PThreadContext
{
    ...
    uint32_t thread_cycles; // total cycles for the entire thread 
        uint32_t recv_blocked_cycles; //the number of cycles the thread is blocked on recv
        uint32_t send_blocked_cycles; //the number of cycles the thread is blocked on recv
